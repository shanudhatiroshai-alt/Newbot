...
async function connectToWA() {
  ...
  conn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      // Fixed: Check if lastDisconnect.error exists before accessing output
...
- *🖤 Thank You for Choosing SHANU-MD!* 
...
╰─🚀 *POWERED BY SHANU TECHX*`
  ...
  conn.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
    ...
    if (options.asSticker || /webp/.test(mime)) {
      ...
      pathFile = await writeExif(media, { packname: config.PACK_NAME || 'SHANU-MD', author: config.AUTHOR_NAME || 'SHANU', categories: options.categories ? options.categories : [] })
  ...
  conn.sendMedia = async(jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
    ...
    if (options.asSticker || /webp/.test(mime)) {
      ...
      let media = { mimetype: mime, data }
      pathFile = await writeExif(media, { packname: options.packname ? options.packname : config.PACK_NAME || 'SHANU-MD', author: options.author ? options.author : config.AUTHOR_NAME || 'SHANU', categories: options.categories ? options.categories : [] })
      await fs.promises.unlink(filename)
      type = 'sticker'
      mimetype = 'image/webp'
    } else if (/image/.test(mime)) type = 'image'
  ...
  conn.sendImageAsSticker = async (jid, buff, options = {}) => {
    ...
    if (options && (options.packname || options.author)) {
      ...
      const { imageToWebp } = require('./lib/exif')
    ...
    for (let i of kon) {
      list.push({
        ...
        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await conn.getName(
          i + '@s.whatsapp.net',
        )}\nFN:${config.OWNER_NAME || 'SHANU'}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.EMAIL;type=INTERNET:${config.EMAIL || 'support@shanu.tech'}\nitem2.X-ABLabel:Email\nitem3.URL:${config.GITHUB_URL || 'https://github.com/Shanudhatirosh'}\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${config.LOCATION || 'Sri Lanka'};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`,
      })
    }
    conn.sendMessage(
      jid,
